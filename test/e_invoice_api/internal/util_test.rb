...
class EInvoiceAPI::Test::UtilFormDataEncodingTest < Minitest::Test
  ...
  def test_encoding_length
    headers, = EInvoiceAPI::Internal::Util.encode_content(
  ...
  def test_file_encode
    ...
    fileinput = EInvoiceAPI::Internal::Type::Converter.dump(EInvoiceAPI::Internal::Type::FileInput, "abc")
    ...
    cases = {
      ...
      file => [file.basename.to_path, /^class EInvoiceAPI/],
      EInvoiceAPI::FilePart.new(file, filename: "d o g") => ["d%20o%20g", /^class EInvoiceAPI/]
    }
  ...
  def test_hash_encode
    ...
    cases = {
      ...
      {pathname: EInvoiceAPI::FilePart.new(Pathname(__FILE__))} => {"pathname" => -> { _1.read in /^class EInvoiceAPI/ }}
    }
    cases.each do |body, testcase|
      encoded = EInvoiceAPI::Internal::Util.encode_content(headers, body)
      cgi = FakeCGI.new(*encoded)
...
class EInvoiceAPI::Test::UtilFusedEnumTest < Minitest::Test
  def test_rewind_closing
    ...
    fused = EInvoiceAPI::Internal::Util.fused_enum(enum, external: true) do
      touched = true
      loop { enum.next }
    end
  ...
  def test_thread_interrupts
    ...
    fused_1 = EInvoiceAPI::Internal::Util.fused_enum(enum, external: true) { loop { enum.next } }
    fused_2 = EInvoiceAPI::Internal::Util.chain_fused(fused_1) { fused_1.each(&_1) }
    fused_3 = EInvoiceAPI::Internal::Util.chain_fused(fused_2) { fused_2.each(&_1) }
  ...
  def test_external_iteration
    iter = [1, 2, 3].to_enum
    first = iter.next
    fused = EInvoiceAPI::Internal::Util.fused_enum(iter, external: true)
